...
//    val sku: TextView,
    ...
    fun bind(product: ProductModel) {
        ...
        val detailsInfo = when {
            product.purchase != null     -> buildString {
                appendln("purchasedAt: ${buildTime(product.purchase.purchasedAt)}")
                appendln("canceledAt: ${product.purchase.canceledAt?.let { buildTime(it) }}")
            }
            product.subscription != null -> buildString {
                appendln("startedAt: ${buildTime(product.subscription.startedAt)}")
                appendln("expiresAt: ${buildTime(product.subscription.expiresAt)}")
                appendln("status: ${product.subscription.status}")
                appendln("introductory activated: ${product.subscription.isIntroductoryActivated}")
            }
            else                         -> null
        }
...
//        freeTrialPeriod.text = "freeTrialPeriod: " + product.details?.freeTrialPeriod
        ...
        introductoryPriceAmountMicros.text =
            "introductoryPriceAmountMicros: " + product.details?.introductoryPriceAmountMicros?.toString()
...
//        introductoryPricePeriod.text = "introductoryPricePeriod: " + product.details?.introductoryPricePeriod
        ...
        originalPriceAmountMicros.text =
            "originalPriceAmountMicros: " + product.details?.originalPriceAmountMicros?.toString()
        price.text = "price: " + product.details?.price
        priceAmountMicros.text =
            "priceAMountMicros: " + product.details?.priceAmountMicros?.toString()
