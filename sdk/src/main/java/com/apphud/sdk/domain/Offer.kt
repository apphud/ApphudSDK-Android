...
class Offer(skuDetails: SkuDetails, offerTokenId: String) {
    val base_plan_id: String? =
        skuDetails.subscriptionOfferDetails?.find { it.offerToken == offerTokenId }?.basePlanId
    val offer_id: String? =
        skuDetails.subscriptionOfferDetails?.find { it.offerToken == offerTokenId }?.offerId
