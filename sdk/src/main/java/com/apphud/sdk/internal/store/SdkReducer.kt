...
internal fun sdkReducer(
    state: SdkState,
    event: SdkEvent,
): Pair<SdkState, List<SdkEffect>> {
    if (event is SdkEvent.SessionCleared) {
        return SdkState.NotInitialized to emptyList()
    }
    ...
    return when (state) {
        is SdkState.NotInitialized -> when (event) {
            is SdkEvent.StartInitialization -> {
                if (event.needRegistration) {
                    SdkState.Registering(event.apiKey, event.userId, isForce = true, isNew = event.isNew) to
                        listOf(SdkEffect.PerformRegistration(isForce = true, isNew = event.isNew))
                } else {
        ...
        is SdkState.Ready -> when (event) {
            ...
            is SdkEvent.ForceRegistrationRequested -> {
                SdkState.Registering(state.apiKey, event.userId, isForce = true, isNew = false) to
